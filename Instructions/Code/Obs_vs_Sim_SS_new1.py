...
directory_S = PROJECT_ROOT / "01_Steady_Calibrate" / "Out" / "run_flopy_shp"
...
fig = plt.figure(figsize=(12, 10))   # Larger canvas to fit 4 subplots
gs = gridspec.GridSpec(2, 2, figure=fig, height_ratios=[1, 1], width_ratios=[1, 1])
gs.update(hspace=0.3, wspace=0.3) # Spacing between subplots

ax1 = fig.add_subplot(gs[0, 0])    # Top-left: pumping parity plot
ax2 = fig.add_subplot(gs[0, 1])    # Top-right: natural parity plot
ax3 = fig.add_subplot(gs[1, 0])    # Bottom-left: natural line plot
ax4 = fig.add_subplot(gs[1, 1])    # Bottom-right: pumping line plot

# ---------- Subplot 1: Pumping Parity Plot ----------
ax1.plot([min_val_pmp, max_val_pmp], [min_val_pmp, max_val_pmp], 'k--', alpha=0.5, linewidth=1.0, label='1:1 line')
sc1 = ax1.scatter(obs_pmp, sim_pmp, c=resid_pmp, cmap='RdYlGn_r',
         s=55, edgecolors='black', linewidth=0.4, alpha=0.85)
cbar1 = fig.colorbar(sc1, ax=ax1, fraction=0.046, pad=0.04)
cbar1.set_label('Residual (m)', fontsize=8)
ax1.set_xlabel('Observed Head (m) - pumping', fontsize=9)
ax1.set_ylabel('Simulated Head (m) - pumping', fontsize=9)
ax1.set_title('Parity Plot (Pumping)', fontsize=10, fontweight='bold')
ax1.grid(True, alpha=0.3)
ax1.legend(loc='lower right', fontsize=8)
ax1.set_aspect('equal', adjustable='box')

textstr1 = f'R²={r2_pmp:.3f}\nRMSE={rmse_pmp:.3f} m\nMAE={mae_pmp:.3f} m'
ax1.text(0.04, 0.96, textstr1, transform=ax1.transAxes, fontsize=8,
     va='top', ha='left',
     bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.75, linewidth=0.5))

# ---------- Subplot 2: Natural Parity Plot ----------
ax2.plot([min_val_nat, max_val_nat], [min_val_nat, max_val_nat], 'k--', alpha=0.5, linewidth=1.0, label='1:1 line')
sc2 = ax2.scatter(obs_nat, sim_nat, c=resid_nat, cmap='RdYlGn_r',
         s=55, edgecolors='black', linewidth=0.4, alpha=0.85)
cbar2 = fig.colorbar(sc2, ax=ax2, fraction=0.046, pad=0.04)
cbar2.set_label('Residual (m)', fontsize=8)
ax2.set_xlabel('Observed Head (m) - natural', fontsize=9)
ax2.set_ylabel('Simulated Head (m) - natural', fontsize=9)
ax2.set_title('Parity Plot (Natural)', fontsize=10, fontweight='bold')
ax2.grid(True, alpha=0.3)
ax2.legend(loc='lower right', fontsize=8)
ax2.set_aspect('equal', adjustable='box')

textstr2 = f'R²={r2_nat:.3f}\nRMSE={rmse_nat:.3f} m\nMAE={mae_nat:.3f} m'
ax2.text(0.04, 0.96, textstr2, transform=ax2.transAxes, fontsize=8,
     va='top', ha='left',
     bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.75, linewidth=0.5))

# ---------- Subplot 3: Natural Line Plot ----------
ax3.plot(x, obs_nat, marker='o', markersize=4, linewidth=1.2, label='Observed')
ax3.plot(x, sim_nat, marker='s', markersize=4, linewidth=1.2, label='Simulated')
ax3.set_title('Heads Comparison - Natural', fontsize=10, fontweight='bold')
ax3.set_xlabel('Observation Points', fontsize=9)
ax3.set_ylabel('Head (m)', fontsize=9)
ax3.grid(True, alpha=0.3)
ax3.legend(loc='best', fontsize=8)
ax3.set_xticks(x)
ax3.set_xticklabels(labels, rotation=0, fontsize=8)

# ---------- Subplot 4: Pumping Line Plot ----------
ax4.plot(x, obs_pmp, marker='o', markersize=4, linewidth=1.2, label='Observed')
ax4.plot(x, sim_pmp, marker='s', markersize=4, linewidth=1.2, label='Simulated')
ax4.set_title('Heads Comparison - Pumping', fontsize=10, fontweight='bold')
ax4.set_xlabel('Observation Points', fontsize=9)
ax4.set_ylabel('Head (m)', fontsize=9)
